#!/bin/bash
# Скрипт для быстрого обновления сервера

echo "🔄 Обновление сервера MasterProfi..."

# Переходим в директорию backend
cd "$(dirname "$0")"

# Останавливаем серверы
echo "⏸️  Останавливаем серверы..."
pm2 stop bestapp-backend || pm2 stop server || true
pm2 stop admin-panel || true

# Получаем последние изменения
echo "📥 Загружаем изменения из GitHub..."
git pull origin main

# Устанавливаем зависимости (если есть новые)
echo "📦 Проверяем зависимости backend..."
npm install --production

# Обновляем админ-панель (если нужно)
if [ -d "admin-panel" ]; then
    echo "📦 Обновляем админ-панель..."
    cd admin-panel
    npm install
    npm run build
    cd ..
    echo "✅ Админ-панель собрана"
    
    # Запускаем админ-панель отдельно
    echo "▶️  Запускаем админ-панель на порту 3001..."
    cd admin-panel
    if pm2 list | grep -q "admin-panel"; then
        pm2 restart admin-panel
    else
        pm2 start npm --name "admin-panel" -- run serve
    fi
    cd ..
fi

# Запускаем сервер
echo "▶️  Запускаем сервер..."
if pm2 list | grep -q "bestapp-backend"; then
    pm2 restart bestapp-backend
elif pm2 list | grep -q "server"; then
    pm2 restart server
else
    pm2 start server.js --name "bestapp-backend"
fi

# Сохраняем конфигурацию PM2
pm2 save

echo "✅ Сервер успешно обновлен!"
echo ""
echo "📊 Статус процессов:"
pm2 list

echo ""
echo "📝 Последние логи бэкенда (нажмите Ctrl+C для выхода):"
pm2 logs bestapp-backend --lines 20 || pm2 logs server --lines 20
echo ""
echo "💡 Для просмотра логов админ-панели: pm2 logs admin-panel"
